...
export function commitRoom(dungeon: Dungeon, startX: number, startY: number, endX: number, endY: number): void {
  ...
  for (let y = minY; y <= maxY; y++) { const row = dungeon.floor[y]; if (!row) continue; for (let x = minX; x <= maxX; x++) row[x] = 1; }
...
export function commitDelete(dungeon: Dungeon, startX: number, startY: number, endX: number, endY: number): void {
  ...
  for (let y = minY; y <= maxY; y++) {
    const floorRow = dungeon.floor[y], secretRow = dungeon.secretFloor?.[y];
    for (let x = minX; x <= maxX; x++) {
      if (floorRow) floorRow[x] = 0;
      if (secretRow) secretRow[x] = 0;
