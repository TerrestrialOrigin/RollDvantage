...
- *Prototype (task 1) — isolated result:* content zoom-fill fills 99%×99% of every sheet in real Firefox and Chromium *when each page is a clean `height:100%` child of an `html→body` chain*.
- *Integration result (group-4 checkpoint) — the real multi-page structure breaks it:* with the app's actual DOM (`.page-wrap`, `#contents-key`, per-section content), Firefox resolves `height:100%` to the real page box **only for the first page**; pages after a forced break fall back to content height (page 2 overfills, page 3 under-fills/blanks). This is a Firefox `%`-height-in-fragmentation limit the isolated spike didn't surface. **Full print content zoom-fill is therefore NOT adopted.**
- *Decision (pragmatic):* keep the shipped print model — the `position:fixed` overlay border + flowed content + `break-after`/`.sheet-last` — which reliably auto-fits border/breaks/margins on **any** paper in both engines. Content is printed at a consistent, correctly-framed size (not zoomed to fill a larger sheet). **On-screen** content scaling (map fills the frame at the selected size) delivers the visible "resize" experience. Print content zoom-fill is deferred as future work.
